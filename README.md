...
```
 ...
 delete user
```
curl -X DELETE http://localhost:8080/users/delete/admin -H 'Authorization: Bearer <JWT token>'
```

```
curl -X POST "http://localhost:8080/users/signup" -H "Authorization: Bearer <JWT token>" -H "Content-Type: application/json" -d '{ "username": "brian", "email": "brian@test.com", "password": "pass", "appUserRoles": [ "ROLE_ADMIN" ]}'
```
